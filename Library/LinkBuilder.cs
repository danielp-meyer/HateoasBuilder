...
namespace MeyerCorp.HateoasBuilder
{
    public class LinkBuilder
    {
        ...
        {
            return RelHrefPairs
                .Select(p => new Link(p.Item1.Trim(), encode
        ...
        public LinkBuilder AddRouteLink(string relLabel, string relativeUrl, params object[] routeItems)
        {
            ...
            var items = routeItems.Select(ri => ri?.ToString().Trim());
        ...
        public LinkBuilder AddParameters(params object[] queryPairs)
        {
            ...
            {
                query.Append($"{queryPairs[index].ToString().Trim()}={queryPairs[index + 1]?.ToString().Trim()}&");
            }

            var queries = query.ToString().Trim().Length > 0
                ? query.ToString().Trim('&')
                : String.Empty;
