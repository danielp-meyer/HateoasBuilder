    ...
    public class LinkBuilder
    {
        ...
        public IEnumerable<Link> Build(bool encode = false)
        {
            return RelHrefPairs.Select(p =>
            {
                ...
                {
                    if (p.Item2.IsExternal)
                        url.Append(relativeurl);
                    else
                        url.AppendFormat("/{0}", relativeurl);
                }
        ...
        /// </remarks>
        public LinkBuilder AddExternalLink(bool condition, string relLabel, string relPathFormat, params object[] formatItems)
        {
            if (condition)
            {
                var newurl = String.Format(relPathFormat, formatItems);
                var newinfo = new LinkInformation(newurl, true);
                var newlink = new Tuple<string, LinkInformation>(relLabel, newinfo);

                RelHrefPairs.Add(newlink);
            }
